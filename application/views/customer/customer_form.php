	...
	<div class="box">
		<div class="box-header">
			...
			<div class="pull-right">
				<a href="<?= site_url('customer'); ?>" class="btn btn-primary btn-sm"><i class="fa fa-undo"></i> Back</a>
			</div>
		</div>
		<div class="box-body">
			<div class="row">
				<div class="col-md-4 col-md-offset-4">
					...
					<form action="<?= site_url('customer/proccess') ?>" method="POST">
						...
						<div class="form-group <?= form_error('name') ? 'has-error' : null ?>">
							...
							<input type="text" name="name" value="<?= $customer->name ?>" class="form-control" id="name" autofocus required>
							<?= form_error('name') ?>
						</div>
						...
						<div class="form-group <?= form_error('address') ? 'has-error' : null ?>">
							...
							<textarea name="address" class="form-control" id="address" cols="30" rows="2" required><?= $customer->address ?></textarea>
							<?= form_error('address') ?>
						</div>
						<div class="form-group <?= form_error('phone') ? 'has-error' : null ?>">
							<label for="phone">Phone *</label>
							<input type="number" name="phone" value="<?= $customer->phone ?>" class="form-control" id="phone" required>
							<?= form_error('phone') ?>
						</div>
